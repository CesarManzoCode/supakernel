/**
 * A dependency-free, synchronous SHA-256 (contract §7 — the core runs on Node, Bun, Deno,
 * workerd and the browser with no Node built-ins and no polyfill). WebCrypto's `subtle.digest`
 * is async and cannot back the synchronous fingerprint / ETag paths, so this is the portable
 * primitive. FIPS 180-4; verified against the standard test vectors.
 */
